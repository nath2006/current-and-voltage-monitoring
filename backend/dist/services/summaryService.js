...
async function getSummary(period = "today") {
    ...
    return {
        ...
        change: {
            totalEnergyKwh: percentChange(previousAgg.totalEnergyKwh, currentAgg.totalEnergyKwh),
            activeDurationSeconds: percentChange(previousAgg.activeDurationSeconds, currentAgg.activeDurationSeconds),
        },
    };
}
function aggregate(summaries) {
    ...
    const activeDurationSeconds = summaries.reduce((s, d) => s + d.activeDurationSeconds, 0);
    const avgPower = summaries.reduce((s, d) => s + d.avgPower, 0) / summaries.length;
